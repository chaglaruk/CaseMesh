...
describe("Matter workspace contract",()=>{
  ...
  it("labels contradiction detection origin without promoting AI analysis to evidence",()=>{expect(page).toContain("detectionOrigin:string");expect(page).toContain("aiAnalysis:boolean");expect(page).toContain("analysisSourceSpanIds:string[]");expect(page).toContain('dispute.aiAnalysis?"AI analysis"');expect(page).toContain('dispute.detectionOrigin==="DeterministicRule"');expect(page).toContain("View AI-analysis input source");expect(page).toContain("Canonical conflict");});
