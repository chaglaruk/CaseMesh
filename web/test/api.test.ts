...
describe("BFF client",()=>{
  ...
  it("redirects authentication failures",async()=>{const assign=vi.fn();Object.defineProperty(window,"location",{configurable:true,value:{assign}});vi.stubGlobal("fetch",vi.fn().mockResolvedValue(new Response(null,{status:401})));await expect(request("/private")).rejects.toThrow("Authentication required");expect(assign).toHaveBeenCalledWith("/sign-in");});
});
