...
| Live OpenAI transcription | AUTOMATED_ONLY | Two independent 24 kHz PCM transcription sessions default to `gpt-live-transcribe`, matching the current dedicated OpenAI Realtime Transcription guide; serialized sends, safe errors/close, bounded reconnect, item-chain ordering and duplicate-final suppression compile. Synthetic A/B out-of-order protocol tests pass; no live API session was run. |
