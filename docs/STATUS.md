...
| Core orchestration | AUTOMATED_ONLY | Actual-turn state/SQLite ingestion is separated from cancellable model work. Deterministic tests cover USER persistence during blocked assistance, USER invalidation, newer-HR supersession, late older finals, bounded stop, assistant timeout and source-aware health. High-risk informational turns remain explicitly routed to assistance. |
...
| Live OpenAI transcription | AUTOMATED_ONLY | Two independent 24 kHz PCM sessions retain `gpt-live-transcribe` and now use the dedicated guide's `languages`, `keywords` and low-delay shape. Each session has one sender worker, a 12-frame drop-oldest queue, two-second send bound, per-outage reconnect budget, gap diagnostics, item ordering and duplicate suppression. Protocol/queue/reset tests pass; the application Credential Manager key was absent, so no live API session ran. |
...
| Live start/stop UI | PARTIAL | Teams/microphone selectors, dual capture/transcription, compact actual transcript, stale-guidance clearing, bounded stop and local unfinished-meeting recovery are wired. HR/USER reconnect, transcription degradation/gap, assistant degradation and manual states aggregate deterministically. Live Teams/API exercise remains pending. |
