...
| Live OpenAI transcription | PARTIAL | Live validation reproduced that the WebSocket connection model and `gpt-live-transcribe` session model are separate. The client now uses the current documented Realtime connection model while retaining `gpt-live-transcribe`, 24 kHz PCM, `languages`, `keywords` and low delay in each transcription session. The model errors no longer occurred, but the server returned `insufficient_quota/credit_balance_exhausted`; no final transcript or `item_id` evidence exists yet. |
