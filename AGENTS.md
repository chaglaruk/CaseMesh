...
Build a trustworthy evidence operating system for workplace disputes.

The commercial critical path is:

`evidence -> provenance -> structured Case Brain -> chronology -> dispute map -> correction -> professional handover`
...
1. commercial architecture scaffolding while keeping the existing solution buildable;
2. v2 epistemic domain (`SourceSpan`, `Assertion`, `Event`, `Contradiction`, `AuditEvent`);
3. invariant tests;
...
10. case-grounded Q&A after provenance gates pass.
