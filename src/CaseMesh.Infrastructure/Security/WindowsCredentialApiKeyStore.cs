...
namespace CaseMesh.Infrastructure.Security;

public sealed class WindowsCredentialApiKeyStore : IApiKeyStore
{
    private const string Target = "CaseMesh/OpenAI";
    private const string LegacyTarget = "HRCompanion/OpenAI";
    ...
    public Task<string?> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (value is not null) return Task.FromResult<string?>(value);

        value = Read(LegacyTarget);
        if (value is null) return Task.FromResult<string?>(null);

        // Copy forward without deleting the legacy entry, so an interrupted rename cannot lose the user's key.
        SaveAsync(value, cancellationToken).GetAwaiter().GetResult();
